...
class Wechat {
    ...
    private function validateSignature() {
        ...
        $com = $companyCache->getCompanyByAppId($appId);
        if ($com == null) {
            return false;
        }

        $token = $com->token; //com.getToken();
    ...
    public function run() {
        ...
        SAELog::debug("msg:" . $msg . ", fromUserName:" . $FromUserName . ", toUserName:" . $ToUserName . ", msgtype:" . $msgtype . ", event:" . $event);

        $b = $this->processSpecialMsg();
        if($b){
            return;
        }
        $comcache = new CompanyCache();
        $com = $comcache->getCompanyByCompanyId($ToUserName);
        if ($com == null || strlen($com->companyId) < 5) {
            return $this->responseText('未接入(或接入未完成)');
        }
