 ...
 * @property string $resourceTypeName
 * @property string $reg
...
class Resource extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('companyId, resourceNo', 'required'),
			array('isUsed', 'numerical', 'integerOnly'=>true),
			array('companyId, resourceTypeName', 'length', 'max'=>32),
			array('resourceNo', 'length', 'max'=>5),
			array('reg', 'length', 'max'=>128),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, companyId, resourceNo, resourceTypeName, reg, isUsed', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('resourceNo',$this->resourceNo,true);
		$criteria->compare('resourceTypeName',$this->resourceTypeName,true);
